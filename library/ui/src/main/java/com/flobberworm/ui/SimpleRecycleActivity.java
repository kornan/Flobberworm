...
public abstract class SimpleRecycleActivity extends BaseDagger2Activity implements SwipeRefreshLayout.OnRefreshListener,
        ItemClickSupport.OnItemClickListener, OnLoadMoreListener, LoadManager.OnLoadListener {
    ...
    public void onEnd() {
        ...
        try {
            if (adapter.getItemCount() > 0) {
                loadManager.getmLoadingAndRetryManager().showContent();
            } else {
                loadManager.getmLoadingAndRetryManager().showEmpty();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
